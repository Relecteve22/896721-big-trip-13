...
import {DEFAULT_OFFERS, defaultEvent} from "../const.js";
...
const createAddNewEventTemplate = (event, indexOffersInArr, offersData, pointsRoute, citysDestination) => {
  const {pointRoute, price, dateFrom, dateTo, offers: offersEvent, destination} = event;
...
export default class AddNewEvent extends Abstract {
  constructor(offersInfo, pointsRoute, offerIndexByTypePoint, destinations, citysDestination, event) {
    super();
  ...
  _getTemplate() {
    return createAddNewEventTemplate(this._event, this._offerIndexByTypePoint, this._offersInfo, this._pointsRoute, this._citysDestination);
  }
